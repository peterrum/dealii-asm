...
#include "vector_access_reduced_.h"
...
#define EXPAND_OPERATIONS(OPERATION)                                         \
  ...
  const unsigned int n_q_points_1d =                                         \
    this->data->get_shape_info().data[0].n_q_points_1d;                      \
                                                                             \
  if (degree == 1 && n_q_points_1d == 2)                                     \
    ...
    AssertThrow(false,                                                       \
                ExcNotImplemented(                                           \
                  "Degree " + std::to_string(degree) + " n_q_points " +      \
  ...
  public:
    ...
    ConstraintInfoReduced compressed_rw;
    ...
    void
    initialize(
      std::shared_ptr<const MatrixFree<dim, Number, VectorizedArrayType>> data_)
    {
      compute_time                 = 0.;
      const unsigned int fe_degree = data_->get_dof_handler(0).get_fe().degree;
      fast_read =
        !data_->get_dof_handler(0).get_triangulation().has_hanging_nodes();
      this->data = data_;
      ...
      const AffineConstraints<Number> &constraints =
        data->get_affine_constraints();

      coefficients_eo.resize(
        data_->get_shape_info().data[0].shape_gradients_collocation_eo.size());
      for (unsigned int i = 0; i < coefficients_eo.size(); ++i)
        coefficients_eo[i] =
          data_->get_shape_info().data[0].shape_gradients_collocation_eo[i][0];

      {
        compressed_rw.initialize(*data_);
      }
      ...
      if (false && Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
        {
          ...
          std::vector<unsigned int> distances(
            di.vector_partitioner->locally_owned_size() / 64 + 1,
          ...
          for (unsigned int part = 0; part < ti.partition_row_index.size() - 2;
               ++part)
            {
                   ...
                   ++i)
                {
                  ...
                  for (unsigned int id = di.cell_loop_pre_list_index[i];
                       ...
                       ++id)
                    {
                      for (unsigned int a = di.cell_loop_pre_list[id].first;
                        ...
                        distances[a / 64] = i;
                      // std::cout << id << "[" <<
                      // di.cell_loop_pre_list[id].first << ","
                  ...
                  for (unsigned int id = di.cell_loop_post_list_index[i];
                       ...
                       ++id)
                    {
                      ...
                      // std::cout << id << "[" <<
                      // di.cell_loop_post_list[id].first << ","
      ...
      for (unsigned int c = 0; c < data->n_cell_batches(); ++c)
        {
          for (unsigned int l = 0; l < data->n_active_entries_per_cell_batch(c);
               ++l)
            {
              ...
              const auto v =
                data->get_mapping_info().mapping->get_vertices(cell);
              if (dim == 2)
                {
              ...
              for (unsigned int i2 = 0; i2 < (dim > 2 ? 3 : 1); ++i2)
                {
                  for (unsigned int i1 = 0; i1 < 3; ++i1)
                    for (unsigned int i0 = 0, i = 9 * i2 + 3 * i1; i0 < 3; ++i0)
                      points[i + i0] =
                        coeff[i0] * fe_values_quad.quadrature_point(i) +
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  ...
                  for (unsigned int d = 0; d < dim; ++d)
                    quadrature_points[c * dim * n_q_points + d * n_q_points + q]
                                     [l] = fe_values.quadrature_point(q)[d];
                }
                      ...
                      if (!constraints.is_constrained(dof_indices[cf]))
                        {
                          ...
                          if (dim == 3 && (quad == 2 || quad == 3))
                            for (unsigned int i1 = 0, i = 0; i1 < fe_degree - 1;
                                 ++i1)
        ...
        end_indices:
          ...
          if (fe_degree > 2)
            {
              for (unsigned int i = 0; i < Utilities::pow<unsigned int>(3, dim);
                   ++i)
    ...
    {
      this->data->cell_loop(
        &LaplaceOperator::local_apply_quadratic_geo, this, dst, src, false);
      internal::set_constrained_entries(data->get_constrained_dofs(0),
                                        src,
                                        dst);
    }
      ...
      const Number beta_old) const
    {
      ...
      for (unsigned int i = 0; i < 7; ++i)
        {
          results[i] = sums[i][0];
          for (unsigned int v = 1; v < dealii::VectorizedArray<Number>::size();
               ++v)
            results[i] += sums[i][v];
        }
                           ...
                           const Number beta_old) const
    {
      ...
      this->data->cell_loop(
        ...
        [&](const unsigned int start_range, const unsigned int end_range) {
          for (unsigned int bl = 0; bl < ::internal::get_n_blocks(x); ++bl)
            do_cg_update3b<n_read_components, Number>(
              start_range,
              end_range,
              ::internal::get_block(g, bl).begin(),
              ::internal::get_block(d, bl).begin(),
    ...
    vmult_with_all_cg_updates(const Number alpha,
                              const Number beta,
                              const
    DiagonalMatrix<LinearAlgebra::distributed::Vector<Number>> &prec,
    ...
    {
      Tensor<1,7,VectorizedArray<Number>> sums;
      this->data->cell_loop(&LaplaceOperator::template
    local_apply_linear_geo<false>, this, h, d,
                            [&](const unsigned int start_range,
                                const unsigned int end_range)
                            {
                              do_cg_update4<1,Number,true>(start_range,
    end_range, h.begin(), x.begin(), g.begin(), d.begin(),
                                                           prec.get_vector().begin(),
                                                           alpha, beta);
                            },
                            [&](const unsigned int start_range,
    ...
    prec.get_vector().begin(), sums);
      ...
      dealii::Utilities::MPI::sum(dealii::ArrayView<const
    double>(results.data(), 7), d.get_partitioner()->get_mpi_communicator(),
    ...
    */
    ...
    {
      this->data->cell_loop(
        &LaplaceOperator::local_apply_merged, this, dst, src, true);
      internal::set_constrained_entries(data->get_constrained_dofs(0),
                                        src,
                                        dst);
    }
    ...
    {
      this->data->cell_loop(
        &LaplaceOperator::local_apply_basic, this, dst, src, true);
      internal::set_constrained_entries(data->get_constrained_dofs(0),
                                        src,
                                        dst);
    }
    ...
    {
      this->data->cell_loop(
        &LaplaceOperator::local_apply_construct_q, this, dst, src, true);
      internal::set_constrained_entries(data->get_constrained_dofs(0),
                                        src,
                                        dst);
    }
      ...
      for (unsigned int cell = 0; cell < data->n_cell_batches(); ++cell)
        {
          const VectorizedArrayType *src_ptr =
            src.data() + cell * n_q_points * n_components;
          VectorizedArrayType *dst_ptr =
            dst.data() + cell * n_q_points * n_components;
                ...
                for (unsigned int c = 0; c < n_components; ++c)
                  {
                    ...
                    for (unsigned int d = 0; d < dim; ++d)
                      {
                        tmp[d] =
                          jac[d][0] * phi_grads[qxy + c * 2 * n_q_points_2d] +
                          jac[d][1] *
                            phi_grads[qxy + (c * 2 + 1) * n_q_points_2d] +
                          jac[d][2] *
                            phi_grads[q + 2 * n_components * n_q_points_2d +
                    ...
                    dealii::internal::EvaluatorTensorProduct<
                      ...
                      template apply<0, true, false, 1>(
                        ...
                        quadrature_points.begin() +
                          (cell * dim + d) * n_q_points + qz * n_q_points_2d +
                          qy * n_q_points_1d,
                  ...
                  for (unsigned int qx = 0; qx < n_q_points_1d; ++qx, ++q)
                    {
                      ...
                      for (unsigned int e = 0; e < dim; ++e)
                        jac[1][e] = jacobians_y[e * n_q_points_2d +
                                                qy * n_q_points_1d + qx];
                      for (unsigned int e = 0; e < dim; ++e)
          ...
          for (unsigned int i = 0; i < phi.dofs_per_cell; ++i)
            {
              ...
              phi.submit_dof_value(
                make_vectorized_array<VectorizedArrayType>(1.), i);
        ...
        [&](const MatrixFree<dim, Number, VectorizedArrayType> &data,
          ...
          for (unsigned int cell = cell_range.first; cell < cell_range.second;
               ++cell)
            {
              phi.reinit(cell);
              if (fe_degree > 2)
                for (unsigned int bl = 0; bl < ::internal::get_n_blocks(src);
                     ++bl)
                  read_dof_values_compressed<dim,
                                             fe_degree,
                                             fe_degree + 1,
                                             n_read_components,
                                             Number>(::internal::get_block(src,
                                                                           bl),
                                                     compressed_dof_indices,
                                                     all_indices_uniform,
                                                     cell,
      ...
      if (system_matrix.m() == 0 && system_matrix.n() == 0)
        {
          ...
          SparsityTools::distribute_sparsity_pattern(
            dsp, owned_dofs, dof_handler.get_communicator(), relevant_dofs);
  ...
  private:
    ...
    template <int fe_degree, int n_q_points_1d>
    void
    local_apply_basic_impl(
      VectorType &                                 dst,
      const VectorType &                           src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
    {
      const unsigned int eval_version = 3;

      if (eval_version == 2)
        {
          // version 2: deal.II gather + deal.II eval
      ...
      else if (eval_version == 3)
        {
          ...
          for (unsigned int cell = cell_range.first; cell < cell_range.second;
            ...
            {
              phi.reinit(cell);
              compressed_rw.read_dof_values(src, phi);
              phi.evaluate(EvaluationFlags::gradients);
              for (unsigned int q = 0; q < phi.n_q_points; ++q)
                phi.submit_gradient(phi.get_gradient(q), q);
              phi.integrate(EvaluationFlags::gradients);
              compressed_rw.distribute_local_to_global(dst, phi);
            }

          return;
        }
      else if (eval_version == 1)
        {
          // version 1: opt gather + deal.II eval
          constexpr unsigned int n_read_components =
            IsBlockVector<VectorType>::value ? 1 : n_components;
              ...
              if (fe_degree > 2 && fast_read)
                for (unsigned int bl = 0; bl < ::internal::get_n_blocks(src);
                  ...
                  read_dof_values_compressed<dim,
                    ...
                    true, // do not evaluate in x-direction
              ...
              if (fe_degree > 2 && fast_read)
                for (unsigned int bl = 0; bl < ::internal::get_n_blocks(src);
                  ...
                  distribute_local_to_global_compressed<dim,
                    ...
                    cell,
                    phi.get_shape_info().data[0].shape_values_eo,
                    true, // do not integrate in x-direction
                    phi.begin_values() + bl * n_q_points);
              else
                phi.distribute_local_to_global(dst);
            }
      ...
      // version 0: opt gather_eval_x + opt eval
                  ...
                  for (unsigned int qy = 0; qy < n_q_points_1d; ++qy)
                    {
                      ...
                      if (static_cast<int>(
                            data.get_mapping_info().cell_type[cell]) == 0)
                        {
                          const unsigned int ind = data.get_mapping_info()
                                                     .cell_data[0]
                                                     .data_index_offsets[cell];
                          const VectorizedArrayType J = data.get_mapping_info()
                                                          .cell_data[0]
                                                          .JxW_values[ind];
                          const Tensor<2, dim, VectorizedArrayType> jac =
                            data.get_mapping_info().cell_data[0].jacobians[0]
                                                                          [ind];
                          for (unsigned int qx = 0; qx < n_q_points_1d;
                               ++qx, ++q)
                            {
      ...
      const std::pair<unsigned int, unsigned int> &cell_range) const
    {
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second;
           ++cell)
        {
                  ...
                  for (unsigned int qy = 0; qy < n_q_points_1d; ++qy)
                    {
                      ...
                      x_con += y * (v[4] + z * v[7]);
                      // y-derivative, constant part
                      Tensor<1, dim, VectorizedArrayType> y_con =
                        v[2] + z * v[6];
                      // y-derivative, xi-dependent part
                      Tensor<1, dim, VectorizedArrayType> y_var =
                        v[4] + z * v[7];
                      // z-derivative, constant part
                      Tensor<1, dim, VectorizedArrayType> z_con =
                        v[3] + y * v[6];
                      // z-derivative, variable part
                      Tensor<1, dim, VectorizedArrayType> z_var =
                        v[5] + y * v[7];
                      double q_weight_tmp =
                        quad_1d.weight(qz) * quad_1d.weight(qy);
                      for (unsigned int qx = 0; qx < n_q_points_1d; ++qx, ++q)
          ...
          VectorizedArrayType
            scratch[do_sum ? Utilities::pow(fe_degree + 1, dim) * n_components :
                             1];
          if (do_sum)
            {
              phi.read_dof_values(src);
              for (unsigned int i = 0;
                   i < Utilities::pow(fe_degree + 1, dim) * n_components;
                   ++i)
    ...
    template <int fe_degree, int n_q_points_1d>
    void
    local_apply_quadratic_geo_impl(
      VectorType &                                 dst,
      const VectorType &                           src,
      const std::pair<unsigned int, unsigned int> &cell_range) const
    {
                    ...
                    for (unsigned int c = 0; c < n_components; ++c)
                      {
                        ...
                        VectorizedArrayType tmp1 =
                          phi_grads[qxy + (c * 2 + 1) * n_q_points_2d];
                        phi_grads[qxy + c * 2 * n_q_points_2d] =
                          (merged_coefficients[cell * n_q_points + q][0] *
                             tmp0 +
                        ...
                        phi_grads[qxy + (c * 2 + 1) * n_q_points_2d] =
                          (merged_coefficients[cell * n_q_points + q][1] *
                             tmp0 +
                           merged_coefficients[cell * n_q_points + q][3] *
                             tmp1 +
                           merged_coefficients[cell * n_q_points + q][4] *
                             phi_grads[q + 2 * n_components * n_q_points_2d +
                                       c * n_q_points]);
      ...
      const std::pair<unsigned int, unsigned int> &cell_range) const
    {
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second;
           ++cell)
        {
          ...
          //  for (unsigned int bl = 0; bl < ::internal::get_n_blocks(src);
          //  ++bl)
          //    read_dof_values_compressed<dim, fe_degree, n_read_components,
          //    value_type>(
          //      ::internal::get_block(src, bl),
          //      compressed_dof_indices,
          //      all_indices_uniform,
          //      cell,
          //      phi.begin_dof_values() + bl * Utilities::pow(fe_degree + 1,
          //      dim));
          // else
          phi.read_dof_values(src);
          phi.evaluate(EvaluationFlags::gradients);
            ...
            for (unsigned int d = 0; d < dim; ++d)
              dealii::internal::EvaluatorTensorProduct<
                ...
                template apply<2, true, false, 1>(
                  ...
                  quadrature_points.begin() + (cell * dim + d) * n_q_points,
                  jacobians_z + d * n_q_points);
          for (unsigned int q2 = 0, q = 0; q2 < (dim == 3 ? n_q_points_1d : 1);
               ++q2)
            {
              constexpr unsigned int n_q_points_2d =
                n_q_points_1d * n_q_points_1d;
          ...
          //  for (unsigned int bl = 0; bl < ::internal::get_n_blocks(src);
          //  ++bl)
          //    distribute_local_to_global_compressed<dim, fe_degree,
          //    n_read_components, Number>(
          //      ::internal::get_block(dst, bl),
          //      compressed_dof_indices,
          //      all_indices_uniform,
          //      cell,
