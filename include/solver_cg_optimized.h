...
cg_update1(dealii::LinearAlgebra::distributed::Vector<Number> &      r,
           ...
           const Number              alpha)
{
  const dealii::LinearAlgebra::distributed::Vector<Number> &prec =
  ...
  for (unsigned int i =
         (r.locally_owned_size() / dealii::VectorizedArray<Number>::size()) *
       ...
       ++i)
    {
      ...
      prod_gh[0] += r.local_element(i) * prec.local_element(i / n_components) *
                    r.local_element(i);
    }
           ...
           const Number                                       alpha)
{
  ...
  for (unsigned int d = 0; d < n_components; ++d)
    arr_r[d] =
      reinterpret_cast<dealii::VectorizedArray<Number> *>(r.block(d).begin());
  const dealii::VectorizedArray<Number> *arr_h[n_components];
  for (unsigned int d = 0; d < n_components; ++d)
    arr_h[d] = reinterpret_cast<const dealii::VectorizedArray<Number> *>(
      h.block(d).begin());
  const dealii::VectorizedArray<Number> *arr_prec =
    reinterpret_cast<const dealii::VectorizedArray<Number> *>(prec.begin());
  for (unsigned int i = 0; i < r.block(0).locally_owned_size() /
                                 dealii::VectorizedArray<Number>::size();
       ++i)
    for (unsigned int d = 0; d < n_components; ++d)
      {
  ...
  for (unsigned int i = (r.block(0).locally_owned_size() /
                         dealii::VectorizedArray<Number>::size()) *
                        dealii::VectorizedArray<Number>::size();
    ...
    for (unsigned int d = 0; d < n_components; ++d)
      {
        ...
        prod_gh[0] += r.block(d).local_element(i) * prec.local_element(i) *
                      r.block(d).local_element(i);
      }
  dealii::Tensor<1, 2> results;
  results[0] = norm_r[0];
  for (unsigned int v = 1; v < dealii::VectorizedArray<Number>::size(); ++v)
           ...
           const Number              beta)
{
  ...
  for (unsigned int i =
       ...
       ++i)
    {
      ...
      p.local_element(i) =
        beta * p.local_element(i) -
        prec.local_element(i / n_components) * r.local_element(i);
    }
}
           ...
           const Number                                       beta)
{
  ...
  for (unsigned int d = 0; d < n_components; ++d)
    arr_r[d] = reinterpret_cast<const dealii::VectorizedArray<Number> *>(
      r.block(d).begin());
  dealii::VectorizedArray<Number> *arr_x[n_components];
  for (unsigned int d = 0; d < n_components; ++d)
    arr_x[d] =
      reinterpret_cast<dealii::VectorizedArray<Number> *>(x.block(d).begin());
  dealii::VectorizedArray<Number> *arr_p[n_components];
  for (unsigned int d = 0; d < n_components; ++d)
    arr_p[d] =
      reinterpret_cast<dealii::VectorizedArray<Number> *>(p.block(d).begin());
  const dealii::VectorizedArray<Number> *arr_prec =
    reinterpret_cast<const dealii::VectorizedArray<Number> *>(prec.begin());
  for (unsigned int i = 0; i < r.block(0).locally_owned_size() /
                                 dealii::VectorizedArray<Number>::size();
    ...
    for (unsigned int d = 0; d < n_components; ++d)
      {
        ...
        p.block(d).local_element(i) =
          beta * p.block(d).local_element(i) -
          prec.local_element(i) * r.block(d).local_element(i);
      }
}
    ...
    while (conv == dealii::SolverControl::iterate)
      {
        ...
        alpha =
          dealii::Utilities::MPI::sum(alpha, internal::get_communicator(d));

        Assert(std::abs(alpha) != 0., dealii::ExcDivideByZero());
        alpha = gh / alpha;

        const dealii::Tensor<1, 2> results =
          cg_update1<MatrixType::n_components, number>(g,
                                                       h,
                                                       preconditioner,
                                                       alpha);
        ...
        cg_update2<MatrixType::n_components, number>(
          x, g, d, preconditioner, alpha, beta);
      }
...
cg_update3(
  ...
  const dealii::DiagonalMatrix<
    dealii::LinearAlgebra::distributed::Vector<Number>> &preconditioner)
{
  ...
  for (unsigned int i =
       ...
       ++i)
    {
      ...
      sums[5][0] +=
        r.local_element(i) * prec.local_element(i) * r.local_element(i);
               ...
               dealii::Tensor<1, 7, dealii::VectorizedArray<Number>> &sums)
{
  ...
  for (unsigned int i = start / dealii::VectorizedArray<Number>::size();
       ...
       ++i)
    {
      dealii::VectorizedArray<Number> arr_prec;
      for (unsigned int v = 0; v < dealii::VectorizedArray<Number>::size(); ++v)
        arr_prec[v] = prec[(i * dealii::VectorizedArray<Number>::size() + v) /
                           n_components];
...
{
  dealii::VectorizedArray<Number> *arr_p =
    reinterpret_cast<dealii::VectorizedArray<Number> *>(p);
  dealii::VectorizedArray<Number> *arr_x =
    reinterpret_cast<dealii::VectorizedArray<Number> *>(x);
  dealii::VectorizedArray<Number> *arr_r =
    reinterpret_cast<dealii::VectorizedArray<Number> *>(r);
  dealii::VectorizedArray<Number> *arr_h =
    reinterpret_cast<dealii::VectorizedArray<Number> *>(h);
      ...
      for (unsigned int i = start / dealii::VectorizedArray<Number>::size();
           ...
           ++i)
        {
          ...
          for (unsigned int v = 0; v < dealii::VectorizedArray<Number>::size();
               ++v)
            arr_prec[v] =
              prec[(i * dealii::VectorizedArray<Number>::size() + v) /
                   n_components];
      ...
      for (unsigned int i = (end / dealii::VectorizedArray<Number>::size()) *
           ...
           ++i)
        {
          x[i] += alpha_plus_alpha_old * p[i] +
                  alpha_old_beta_old * prec[i / n_components] * r[i];
          r[i] += alpha * h[i];
          p[i] = beta * p[i] - prec[i / n_components] * r[i];
          if (do_update_h)
            h[i] = 0.;
...
{
  static_assert(n_components == 1, "Only single component support");
  do_cg_update4<n_components, Number, false>(
    0,
    r.locally_owned_size(),
    h.begin(),
    x.begin(),
    ...
    while (conv == dealii::SolverControl::iterate)
      {
        ...
        const dealii::Tensor<1, 7> results =
          cg_update3b<MatrixType::n_components, number>(g,
                                                        d,
                                                        h,
                                                        preconditioner);
#else
        ...
        const dealii::Tensor<1, 7> results =
          cg_update3<MatrixType::n_components, number>(loc_sum_hd,
                                                       g,
                                                       h,
                                                       preconditioner);
#endif
        ...
        cg_update4<MatrixType::n_components, number>(
          h, x, g, d, preconditioner, alpha, beta);
      }
                ...
                for (unsigned int bl = 0; bl < internal::get_n_blocks(x); ++bl)
                  {
                    constexpr unsigned int n_components =
                      dealii::IsBlockVector<VectorType>::value ?
                        1 :
                        MatrixType::n_components;
                    dealii::VectorizedArray<number> *arr_p =
                    ...
                    const number alpha_old_beta_old = alpha_old / beta_old;
                    const unsigned int end =
                      internal::get_block(g, bl).locally_owned_size();
                    for (unsigned int i = 0;
                         i < end / dealii::VectorizedArray<number>::size();
                         ++i)
                      {
                    ...
                    for (unsigned int i =
                      ...
                      internal::get_block(x, bl).local_element(i) +=
                        alpha_plus_alpha_old *
                          internal::get_block(d, bl).local_element(i) +
                        alpha_old_beta_old *
                          preconditioner.get_vector().local_element(
                            i / n_components) *
